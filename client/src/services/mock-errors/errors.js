...
import sendChangeEvent from '../pagerduty/sendChangeEvent';
...
// Change Event which "causes" the issue to happen
export const pipelineDeployment = () => {
  let payload = {
    routing_key: PD_CONSTANTS.CHANGE_EVENTS_KEY_WEBSITE_SERVICE,
    payload: {
      summary: `Merge Pull Request #${getRndInteger(
        100,
        1000
      )} from BankDuty/FIX-${getRndInteger(100, 1000)}`,
...
export const highCPU = () => {
  for (let idx = 0; idx < 25; idx++) {
    let payload = {
      routing_key: PD_CONSTANTS.ROUTING_KEY,
      dedup_key: `${PD_CONSTANTS.CLIENT}_datadog_high_cpu_${idx}`,
      event_action: 'trigger',
      client: PD_CONSTANTS.CLIENT,
      client_url: PD_CONSTANTS.CLIENT_URL,
      payload: {
        summary: `Warning: Increased Response Time Detected - Avg ${getRndInteger(
          8000,
          15000
        )}ms | ${getRndInteger(80, 95)}% CPU`,
        source: 'Datadog',
        severity: 'warning',
        component: 'website',
        group: 'prod',
